...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $check_stmt = $pdo->prepare("SELECT id_parecer_medico FROM parecer_medico WHERE id_triagem = :id_triagem");
        $check_stmt->execute([':id_triagem' => $id_triagem_sessao]);
        $parecer_id = $check_stmt->fetchColumn();

        if ($parecer_id) {
            $stmt = $pdo->prepare("UPDATE parecer_medico SET comentario = :comentario, parecer_final_medico = :parecer_final, id_usuario = :id_usuario, data_finalizacao_parecer_medico = NOW() WHERE id_parecer_medico = :id_parecer_medico");
            $stmt->execute([':comentario' => $comentario, ':parecer_final' => $parecer_final, ':id_usuario' => $id_usuario_logado, ':id_parecer_medico' => $parecer_id]);
        } else {
            $stmt = $pdo->prepare("INSERT INTO parecer_medico (id_triagem, id_usuario, comentario, parecer_final_medico, data_finalizacao_parecer_medico) VALUES (:id_triagem, :id_usuario, :comentario, :parecer_final, NOW())");
            $stmt->execute([':id_triagem' => $id_triagem_sessao, ':id_usuario' => $id_usuario_logado, ':comentario' => $comentario, ':parecer_final' => $parecer_final]);
        }
        ...
        if (isset($_POST['avancar_etapa'])) {
            $stmt_triagem = $pdo->prepare("UPDATE triagens SET etapa_atual = 'Parecer Psicológico' WHERE id_triagem = :id_triagem");
...
$etapa_atual_bd = '';
try {
    $stmt_parecer = $pdo->prepare("SELECT * FROM parecer_medico WHERE id_triagem = :id_triagem");
    $stmt_parecer->execute([':id_triagem' => $id_triagem_sessao]);
    $parecer_medico = $stmt_parecer->fetch(PDO::FETCH_ASSOC);
    if(!$parecer_medico) $parecer_medico = [];
        ...
        <main class="main-content">
            ...
            <div class="triagem-layout">
                <aside class="triagem-sidebar">
                    ...
                    <a href="parecer_medico.php<?php echo ($id_idoso_sessao && $id_triagem_sessao) ? "?id_idoso={$id_idoso_sessao}&id_triagem={$id_triagem_sessao}" : "#"; ?>" class="sidebar-button active<?php echo (empty($id_idoso_sessao) ? ' disabled' : ''); ?>">Parecer do Médico</a>
                ...
                <section class="triagem-form-content">
                    <h2>Parecer Médico</h2>
                    <?php if ($mensagem): ?>
                        <div class="admin-message <?= $mensagem_class ?>"><?= html_entity_decode(htmlspecialchars($mensagem)) ?></div>
                    <?php endif; ?>
                    <form method="POST" action="parecer_medico.php">
                        <fieldset>
                            <legend>Comentários adicionais:</legend>
                            <textarea name="comentarios" rows="10" placeholder="Escreva aqui as observações médicas..."><?= htmlspecialchars($parecer_medico['comentario'] ?? '') ?></textarea>
                        </fieldset>
                        <fieldset>
                            <legend>Parecer final</legend>
                            <div class="radio-group">
                                <label><input type="radio" name="parecer_final" value="Aprovado" <?php echo (($parecer_medico['parecer_final_medico'] ?? '') === 'Aprovado') ? 'checked' : ''; ?>> O(A) médico(a) confirma a aprovação do(a) candidato(a).</label>
                                <label><input type="radio" name="parecer_final" value="Rejeitado" <?php echo (($parecer_medico['parecer_final_medico'] ?? '') === 'Rejeitado') ? 'checked' : ''; ?>> O(A) médico(a) confirma a rejeição do(a) candidato(a).</label>
                            </div>
                        </fieldset>
                        <div class="form-buttons">
                            <a href="parecer_diretoria.php?id_idoso=<?= $id_idoso_sessao ?>&id_triagem=<?= $id_triagem_sessao ?>" class="btn-secondary">Voltar</a>
