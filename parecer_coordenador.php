...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($primeiro_parecer_status) {
            $check_stmt = $pdo->prepare("SELECT id_parecer_coordenador_diretoria FROM parecer_coordenador_diretoria WHERE id_triagem = :id_triagem AND tipo = 'Coordenador' AND ordem = 'Primeiro'");
            $check_stmt->execute([':id_triagem' => $id_triagem_sessao]);
            if ($check_stmt->fetch()) {
                $stmt = $pdo->prepare("UPDATE parecer_coordenador_diretoria SET status = :status, observacao = :obs, id_usuario = :id_usuario, responsavel_nome = :responsavel_nome, data_finalizacao_parecer_coord_dir = NOW() WHERE id_triagem = :id_triagem AND tipo = 'Coordenador' AND ordem = 'Primeiro'");
            } else {
                $stmt = $pdo->prepare("INSERT INTO parecer_coordenador_diretoria (id_triagem, tipo, ordem, status, observacao, id_usuario, responsavel_nome, data_finalizacao_parecer_coord_dir) VALUES (:id_triagem, 'Coordenador', 'Primeiro', :status, :obs, :id_usuario, :responsavel_nome, NOW())");
            }
        ...
        if ($segundo_parecer_status) {
            $check_stmt = $pdo->prepare("SELECT id_parecer_coordenador_diretoria FROM parecer_coordenador_diretoria WHERE id_triagem = :id_triagem AND tipo = 'Coordenador' AND ordem = 'Segundo'");
            $check_stmt->execute([':id_triagem' => $id_triagem_sessao]);
            if ($check_stmt->fetch()) {
                $stmt = $pdo->prepare("UPDATE parecer_coordenador_diretoria SET status = :status, observacao = :obs, id_usuario = :id_usuario, responsavel_nome = :responsavel_nome, data_finalizacao_parecer_coord_dir = NOW() WHERE id_triagem = :id_triagem AND tipo = 'Coordenador' AND ordem = 'Segundo'");
            } else {
                $stmt = $pdo->prepare("INSERT INTO parecer_coordenador_diretoria (id_triagem, tipo, ordem, status, observacao, id_usuario, responsavel_nome, data_finalizacao_parecer_coord_dir) VALUES (:id_triagem, 'Coordenador', 'Segundo', :status, :obs, :id_usuario, :responsavel_nome, NOW())");
            }
...
$etapa_atual_bd = '';
try {
    $stmt_pareceres = $pdo->prepare("SELECT * FROM parecer_coordenador_diretoria WHERE id_triagem = :id_triagem AND tipo = 'Coordenador'");
        ...
        <main class="main-content">
            ...
            <div class="triagem-layout">
                ...
                <section class="triagem-form-content">
                    ...
                    <?php endif; ?>
                    <form method="POST" action="parecer_coordenador.php">
                        <div class="parecer-section">
                            <h3>Primeiro parecer:</h3>
                            <div class="radio-group">
                                <label><input type="radio" name="primeiro_parecer_status" value="Aprovado" <?php echo (($pareceres['Primeiro']['status'] ?? '') === 'Aprovado') ? 'checked' : ''; ?>> O(A) coordenador(a) aprova o acolhimento do(a) candidato(a).</label>
                                <label><input type="radio" name="primeiro_parecer_status" value="Rejeitado" <?php echo (($pareceres['Primeiro']['status'] ?? '') === 'Rejeitado') ? 'checked' : ''; ?>> O(A) coordenador(a) rejeita o acolhimento do(a) candidato(a).</label>
                                <label><input type="radio" name="primeiro_parecer_status" value="Lista de Espera" <?php echo (($pareceres['Primeiro']['status'] ?? '') === 'Lista de Espera') ? 'checked' : ''; ?>> O(A) coordenador(a) coloca o candidato(a) na lista de espera.</label>
                            </div>
                        ...
                        <div class="parecer-section">
                            ...
                            <div class="radio-group">
                                <label><input type="radio" name="segundo_parecer_status" value="Aprovado" <?php echo (($pareceres['Segundo']['status'] ?? '') === 'Aprovado') ? 'checked' : ''; ?>> O(A) coordenador(a) confirma a aprovação do(a) candidato(a).</label>
                                <label><input type="radio" name="segundo_parecer_status" value="Rejeitado" <?php echo (($pareceres['Segundo']['status'] ?? '') === 'Rejeitado') ? 'checked' : ''; ?>> O(A) coordenador(a) confirma a rejeição do(a) candidato(a).</label>
                            </div>
                        ...
                        <div class="form-buttons">
                            <a href="ficha_triagem_contrato.php?id_idoso=<?= $id_idoso_sessao ?>&id_triagem=<?= $id_triagem_sessao ?>" class="btn-secondary">Voltar</a>
